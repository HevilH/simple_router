...
namespace simple_router
{
  ...
  {
    printf("Start lookup\r\n");
    const RoutingTableEntry *result = NULL;
    uint32_t pref_ip, pref_dest;
    for (auto &entry : m_entries)
    {
      pref_ip = entry.mask & ip;
      pref_dest = entry.mask & entry.dest;
      if (pref_ip == pref_dest)
      {
        if (result == NULL || entry.mask >= result->mask)
        {
          result = &entry;
        }
      }
